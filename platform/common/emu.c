...
int emu_findBios(int region, char **bios_file)
{
	...
	if (f) {
		...
		//lprintf("%s\n", menuErrorMsg);
		show_msgbox("PicoDrive", "menuErrorMsg");
...
int emu_ReadConfig(int game, int no_defaults)
{
	...
	if (!game)
	{
		...
		if (config_slot != 0)
		{
			...
			sprintf(extbuf, ".%i.pbcfg.tns", config_slot);
		...
		if (config_slot != 0)
		     sprintf(extbuf, ".%i.pbcfg.tns", config_slot);
		else strcpy(extbuf, ".pbcfg.tns");
		romfname_ext(cfg, cfgPath, extbuf);
		f = fopen(cfg, "rb");
		if (!f) romfname_ext(cfg, NULL, ".pbcfg.tns");
		else fclose(f);
	}
...
char *emu_GetSaveFName(int load, int is_sram, int slot)
{
	...
	if (is_sram)
	{
		romfname_ext(saveFname, (PicoMCD&1) ? brmPath : srmPath, (PicoMCD&1) ? ".brm.tns" : ".srm.tns");
		if (load) {
			if (try_ropen_file(saveFname)) return saveFname;
			// try in current dir..
			romfname_ext(saveFname, NULL, (PicoMCD&1) ? ".brm.tns" : ".srm.tns");
			if (try_ropen_file(saveFname)) return saveFname;
			return NULL; // give up
		}
	}
	else
	{
		...
		strcat(ext, (currentConfig.EmuOpt & 8) ? ".mds.gz.tns" : ".mds.tns");
