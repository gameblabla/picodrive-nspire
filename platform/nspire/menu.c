/*
	menu.c - based on platform/gp2x/menu.c
...
void print_text(int x, int y, const char * string);

extern int scandir(const char *dir, struct dirent ***namelist, int (*select) (const struct dirent *), int (*cmp) (const void *, const void *));
...
static unsigned long input2_read(unsigned long interesting, int *joy)
{
	...
	{
		*joy = 0;
		if ((ret = sdl_joystick_read(0) & interesting)) break;
	}
	while(0);
...
static void load_progress_cb(int percent)
{
	...
	unsigned short *dst = (unsigned short *)sdl_screen + 320*20;
...
static void cdload_progress_cb(int percent)
{
	...
	print_text(1, 3*10, "Processing CD image / MP3s");
	print_text(1, 4*10, romFileName);
	dst += 320*30;
...
void menu_romload_prepare(const char *rom_name)
{
	...
	if (rom_data) clearBufferB();
	else memset(sdl_screen, 0, 320*240*2);

	print_text(1, 1, "Loading");
	print_text(1, 10, p);
...
void menu_romload_end(void)
{
	...
	print_text(1, cdload_called ? 60 : 30, "Starting emulation...");
...
static void draw_savestate_menu(int menu_sel, int is_loading)
{
	...
	print_text(tl_x, 30, is_loading ? "Load state" : "Save state");

	
	print_text(tl_x - 16, tl_y + menu_sel*10, ">");
	...
	for (i = 0; i < 10; i++, y+=10)
	{
		...
		{
			case 0:
			print_text(tl_x, y, "SLOT 0");
			break;
			case 1:
			print_text(tl_x, y, "SLOT 1");
			break;
			case 2:
			print_text(tl_x, y, "SLOT 2");
			break;
			case 3:
			print_text(tl_x, y, "SLOT 3");
			break;
			case 4:
			print_text(tl_x, y, "SLOT 4");
			break;
			case 5:
			print_text(tl_x, y, "SLOT 5");
			break;
			case 6:
			print_text(tl_x, y, "SLOT 6");
			break;
			case 7:
			print_text(tl_x, y, "SLOT 7");
			break;
			case 8:
			print_text(tl_x, y, "SLOT 8");
			break;
			case 9:
			print_text(tl_x, y, "SLOT 9");
			break;
		}
		/*text_out16(tl_x, y, "SLOT %i (%s)", i, (state_slot_flags & (1 << i)) ? "USED" : "free");*/
		/*//drawStringF(tl_x, y, 65536, 0, "SLOT %i (%s)", i, (state_slot_flags & (1 << i)) ? "USED" : "free");*/
	}
	print_text(tl_x, y, "back");
...
static void draw_key_config(const bind_action_t *opts, int opt_cnt, int player_idx, int sel)
{
	...
	if (player_idx >= 0) {
		...
		//drawStringF(80, 20, 65536, 0, "Player %i controls", player_idx + 1);
		x = 80;
	} else {
		print_text(80, 20, "Emulator controls");
		x = 40;
	}

	print_text(x - 16, tl_y + sel*10, ">");
	/*menu_draw_selection(x - 16, tl_y + sel*10, (player_idx >= 0) ? 66 : 130);*/

	y = tl_y;
	
	for (i = 0; i < opt_cnt; i++, y+=10)
	//drawStringF(x, y, 65536, 0, "%s : %s", opts[i].name, action_binds(player_idx, opts[i].mask));
		/*text_out16(x, y, "%s : %s", opts[i].name, action_binds(player_idx, opts[i].mask));*/

	print_text(x, y, "Done");

	if (sel < opt_cnt) {
		print_text(30, 180, "Press a button to bind/unbind");
		print_text(30, 190, "Use SELECT to clear");
		print_text(30, 200, "To bind UP/DOWN, hold SELECT");
		print_text(30, 210, "Select \"Done\" to exit");
	} else {
		print_text(30, 190, "Use Options -> Save cfg");
		print_text(30, 200, "to save controls");
		print_text(30, 210, "Press A or B to exit");
	}
	sdl_menu_flip();
}
...
static void draw_kc_sel(int menu_sel)
{
	...
	print_text(tl_x, y,       "Player 1");
	print_text(tl_x, (y+=10), "Player 2");
	print_text(tl_x, (y+=10), "Emulator controls");
	print_text(tl_x, (y+=10), "Done");
...
static void menu_cdopt_cust_draw(const menu_entry *entry, int x, int y, void *param)
{
	...
	switch (entry->id)
	{
		case MA_CDOPT_TESTBIOS_USA: //drawStringF(x, y, 65536, 0, "USA BIOS:     %s", bios_names->us); /*text_out16(x, y, "USA BIOS:     %s", bios_names->us);*/ break;
		case MA_CDOPT_TESTBIOS_EUR: //drawStringF(x, y, 65536, 0, "EUR BIOS:     %s", bios_names->eu); break;
		case MA_CDOPT_TESTBIOS_JAP: //drawStringF(x, y, 65536, 0, "JAP BIOS:     %s", bios_names->jp); break;
		case MA_CDOPT_READAHEAD:
			...
			//drawStringF(x, y, 65536, 0, "ReadAhead buffer      %s", ra_buff);
			break;
		default:break;
	}
}
...
static void draw_cd_menu_options(int menu_sel, struct bios_names_t *bios_names)
{
	...
	for(i=0;i<11;i++)
	{
		print_text(tl_x, tl_y, cdopt_entries[i].name);	
	}
	/*
	...
	*/
	...
	if ((selected_id == MA_CDOPT_TESTBIOS_USA && strcmp(bios_names->us, "NOT FOUND")) ||
		...
		(selected_id == MA_CDOPT_TESTBIOS_JAP && strcmp(bios_names->jp, "NOT FOUND")))
			print_text(tl_x, 210, "Press start to test selected BIOS");
...
static void draw_amenu_options(int menu_sel)
{
	...
	for(i=0;i<7;i++)
	{
		print_text(tl_x, tl_y, opt2_entries[i].name);	
	}
	...
	for(;;)
	{
		...
		if (inp & (GP2X_LEFT|GP2X_RIGHT)) { // multi choise
			if (!me_process(opt2_entries, OPT2_ENTRY_COUNT, selected_id, (inp&GP2X_RIGHT) ? 1 : 0) &&
				...
				while ((inp = sdl_joystick_read(1)) & (GP2X_LEFT|GP2X_RIGHT)) {
...
static void draw_menu_options(int menu_sel)
{
	...
	for(i=0;i<8;i++)
	{
		print_text(tl_x, tl_y+(i*10), opt_entries[i].name);	
	}
...
static void draw_menu_credits(void)
{
	...
	print_text(tl_x, 20, "       PicoDrive SDL v" VERSION);
	print_text(tl_x, 30, " for Ritmix rzx50 and Dingoo a380");
	print_text(tl_x, 40, "       (c) notaz 2006,2007");
	y = tl_y;
	print_text(tl_x, y, "Credits:");
	print_text(tl_x, (y+=10), "fDave: Cyclone 68000 core,");
	print_text(tl_x, (y+=10), "      base code of PicoDrive");
	print_text(tl_x, (y+=10), "Reesy & FluBBa: DrZ80 core");
	print_text(tl_x, (y+=10), "MAME devs: YM2612 and SN76496 cores");
	print_text(tl_x, (y+=10), "Charles MacDonald: Genesis hw docs");
	print_text(tl_x, (y+=10), "Stephane Dallongeville:");
	print_text(tl_x, (y+=10), "      opensource Gens");
	print_text(tl_x, (y+=10), "Haze: Genesis hw info");
	print_text(tl_x, (y+=10), "rlyeh and others: minimal SDK");
	print_text(tl_x, (y+=10), "Squidge: squidgehack");
	print_text(tl_x, (y+=10), "Dzz: ARM940 sample");
	print_text(tl_x, (y+=10), "GnoStiC / Puck2099: USB joystick");
	print_text(tl_x, (y+=10), "craigix: GP2X hardware");
	print_text(tl_x, (y+=10), "ketchupgun: skin design");
...
menu_entry main_entries[] =
{
	...
	{ "Exit",MB_NONE, MA_MAIN_PATCHES,     NULL, 0, 0, 0, 0 },
	{ "Exit",               MB_NONE, MA_MAIN_EXIT,        NULL, 0, 0, 0, 1 }
};

void print_text(int x, int y, const char * string)
{
	drawString(&x, &y, 0, string, 65535, 0);	
}
...
static void menu_prepare_bg(int use_game_bg)
{
	if (use_game_bg)
	{
		...
		memset(sdl_screen, 0, 320*8*2);
		menu_darken_bg((char *)sdl_screen + 320*8*2, 320*224, 1);
		memset((char *)sdl_screen + 320*232*2, 0, 320*8*2);
	}
	/*else
	{
		// should really only happen once, on startup..
		readpng(sdl_screen, "skin/background.png", READPNG_BG);
	}*/

	// copy to buffer2
	sdl_memcpy_buffers((1<<2), sdl_screen, 0, 320*240*2);
}
